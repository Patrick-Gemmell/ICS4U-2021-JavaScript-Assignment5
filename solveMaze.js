...
function createMaze(mazeFile, length, width) {
  ...
  for (let row = 0; row < width; row++) {
    const tempString = mazeFile.substr(createCounter, length);
    const tempArray = [];
...
/**
  * @param {int} boolMaze
  * @param {int} pathTraveled
  * @param {int} finishedMaze
  * @param {int} coordX
  * @param {int} coordY
  * @param {int} goalX
  * @param {int} goalY
...
function exploreMaze(viewMaze, startStop, rowSize, columnSize) {
  ...
  const beginX = startStop[0];
  const beginY = startStop[1];
  const stopX = startStop[2];
  const stopY = startStop[3];

  // Finding a solution for the maze
  const solution = travelMaze(mapBoolean, firstBoolList,
      secondBoolList, beginX,
      beginY, stopX, stopY);
...
const firstMazeFile = fs.readFileSync('Maze1.txt', 'utf8');
const firstMaze = createMaze(firstMazeFile, 6, 6);
...
const firstBreakPoints = findBreakPoints(firstMaze);

// Finding a solution to the first maze and printing it out
const firstSolved = exploreMaze(firstMaze, firstBreakPoints, 6, 6);
printMaze(firstSolved, 6, 6);
console.log('');
console.log('');

// Creating the second maze
const secondMazeFile = fs.readFileSync('Maze2.txt', 'utf8');
const secondMaze = createMaze(secondMazeFile, 6, 12);
...
const secondBreakPoints = findBreakPoints(secondMaze);

// Finding a solution to the second maze and printing it out
const secondSolved = exploreMaze(secondMaze, secondBreakPoints, 6, 12);
printMaze(secondSolved, 6, 12);
console.log('');
console.log('');

// Creating the third maze
const thirdMazeFile = fs.readFileSync('Maze3.txt', 'utf8');
const thirdMaze = createMaze(thirdMazeFile, 19, 9);
...
const thirdBreakPoints = findBreakPoints(thirdMaze);

// Finding a solution to the third maze and printing it out
const thirdSolved = exploreMaze(thirdMaze, thirdBreakPoints, 19, 9);
